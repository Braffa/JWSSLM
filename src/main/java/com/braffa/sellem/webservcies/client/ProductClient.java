...
public class ProductClient extends BaseClient {
	...
	public ClientResponse create(XmlProductMsg xmlProductMsg) {
	...
	public String remove(String productId) {
		...
		try {
			...
			ClientResponse response = webResource.path("rest")
					.path(REMOVE_URI + productId).delete(ClientResponse.class);
	...
	public String delete(String productId) {
		...
		try {
			...
			WebResource webResource = client.resource(REST_URI);
			ClientResponse response = webResource.path("rest")
					.path(DELETE_URI + productId).delete(ClientResponse.class);
			return response.toString();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String findByProductId(String productId) {
		...
		try {
			...
			Client client = Client.create(config);
			WebResource service = client.resource(REST_URI);
			WebResource getService = service.path("rest").path(
					FIND_BY_PRODUCTID_URI + productId);
	...
	public String searchProduct(String field, String value) {
		...
		try {
			...
			WebResource service = client.resource(REST_URI);
			WebResource getService = service.path("rest").path(
					SEARCH_PRODUCT_URI + field + "/" + value);
			return getResponseAsXML(getService);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ClientResponse update(XmlProductMsg xmlProductMsg) {
		if (logger.isDebugEnabled()) {
			logger.debug("update");
		}
		try {
	...
	public ClientResponse remove(XmlProductMsg xmlProductMsg) {
		if (logger.isDebugEnabled()) {
			logger.debug("remove");
		}
		try {
