...
@Path("usertoproduct")
public class UserToProductResource implements IUserToProductWebService {
	@Context
	UriInfo uriInfo;
	private static final Logger logger = Logger
			.getLogger(UserToProductResource.class);
	...
	public String count() {
		if (logger.isDebugEnabled()) {
			logger.debug("countUserToProductMsg");
		}
		try {
	...
	public XmlUserToProductMsg create(XmlUserToProductMsg xmlUserToProductMsg) {
		...
		URI uri = uriInfo.getAbsolutePathBuilder()
				.path(xmlUserToProductMsg.getUserToProduct().getUserId())
				.build();
		Response res = Response.created(uri).build();
		try {
			UserToProductService.getInstance().create(xmlUserToProductMsg);
		} catch (Exception e) {
	...
	@Consumes("application/xml")
	public XmlUserToProductMsg delete(@PathParam("userId") String userId,
		...
		try {
			XmlUserToProductMsg xmlUserToProductMsg = UserToProductService
					.getInstance().delete(userId, productId, productIndex);
			return xmlUserToProductMsg;
		} catch (Exception e) {
	...
	// @Override
	@DELETE
	@Path("/remove/{userId}/{productId}/{productIndex}")
	@Consumes("application/xml")
	public void remove(@PathParam("userId") String userId,
			@PathParam("productId") String productId,
			@PathParam("productIndex") String productIndex) {
		if (logger.isDebugEnabled()) {
			logger.debug("removeUserToProductMsg " + userId);
		}
		try {
			UserToProductService.getInstance().remove(userId, productId,
					productIndex);
		} catch (Exception e) {
	...
	@Produces(MediaType.TEXT_XML)
	public XmlUserToProductMsg find(@PathParam("userId") String userId,
			@PathParam("productId") String productId,
		...
		try {
			return UserToProductService.getInstance().find(userId, productId,
					productIindex);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.TEXT_XML)
	public XmlUserToProductMsg search(@PathParam("searchField") String searchField, @PathParam("value")String value) {
		if (logger.isDebugEnabled()) {
			logger.debug("search");
		}
		try {
			return UserToProductService.getInstance()
